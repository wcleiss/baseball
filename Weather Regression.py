...
def savant_wx_load(yr):
    ...
    savant_outcome_filt.to_csv('savant\\savant_tmp2.csv', index = False, header = True)
...
def savant_wx_parse(yr):
    ...
    savant_wx_merge.to_csv(file_n, index = False, header = True)
...
def savant_wx_assign(yr):
    ...
    wx_group.to_csv(fn_use, index = False, header = True)
...
def savant_wx_binder(syr, eyr):
    ...
    wx_bind.to_csv('weather\\weather_reg_master.csv', index = False, header = True)
...
def wx_neutralize(yr):
    ...
    for st in stand_split:
        ...
        for th in throw_split:
            ...
            wx_slice_sel.to_csv(vla_out, index=False, header=True)
    ...
    for st in stand_split:
        ...
        for th in throw_split:
            ...
            wx_slice_sel = wx_slice_merge[['pitcher', 'double', 'fly_ball', 'ground_ball', 'hr', 'line_drive', 
                                           'popup', 'sacb', 'single', 'strikeout', 'triple', 'walk', 'bf', 'player_type']]
            
            wx_slice_sel.to_csv(vla_out, index=False, header=True)

    wx_bind.to_csv('savant\\wnx_tmp.csv', index=False, header=True)
...
def wn_parkfac(yr):
    ...
    park_factor_use.to_csv(out_file, index=False, header=True)
